...
`정규표현식(Regular Expression)` 혹은 `Regex`란 **특별한 의미를 갖는 문자, 숫자, 기호등을** 조합해 만들어낸, 특정 문자열과 매칭되는 패턴을 의미한다.

---

# **문법**
정규식의 모든 문법을 외우고 사용하는것은 현실적으로 쉽지 않다. 아래 표는 자주 사용되는 기초 문법들을 다룬다.
**[Java Regex](https://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern.html)** 에서 나머지 모든 문법을 확인 할 수 있다.
...
/*
This is the sample text for regex practice. it contains some of email addresses and phone numbers. email addresses contain @, like  **** ,  **** 
phone number could have hyphen like  **** , or not. some people use the dot like  ****  or use just space like  **** 
*/
```
